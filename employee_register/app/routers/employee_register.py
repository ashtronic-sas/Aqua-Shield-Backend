...
from app.crud.employee_register import create_employee_register, update_employee_register, get_registers_by_employee, get_registers_by_place, delete_employee_register
from app.shared.utils import verify_token

router = APIRouter()

# Registrar una nueva entrada de un empleado
@router.post("/employee_register", response_model=EmployeeRegisterResponse, dependencies=[Depends(verify_token)])
...
@router.put("/employee_register/{id}", response_model=EmployeeRegisterResponse, dependencies=[Depends(verify_token)])
def update_employee_exit(id: int, employee_register_update: EmployeeRegisterUpdate, db: Session = Depends(get_db)):
    return update_employee_register(db, id, employee_register_update)

# Obtener historial de asistencia de un empleado
@router.get("/employee_register/employee/{employee_id}", response_model=list[EmployeeRegisterResponse], dependencies=[Depends(verify_token)])
...
@router.get("/employee_register/place/{place_id}", response_model=list[EmployeeRegisterResponse], dependencies=[Depends(verify_token)])
def get_place_registers(place_id: int, db: Session = Depends(get_db)):
    return get_registers_by_place(db, place_id)

# Eliminar un registro de asistencia
@router.delete("/employee_register/{id}", dependencies=[Depends(verify_token)])
