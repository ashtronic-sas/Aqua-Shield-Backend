...
class EmployeeRegister(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    employee_id = Column(Integer, ForeignKey("employee.id"), nullable=False, ondelete='SET NULL'))  # Relación con Employee
    place_id = Column(Integer, ForeignKey("places.id"), nullable=False, ondelete='SET NULL'))  # Relación con Place
