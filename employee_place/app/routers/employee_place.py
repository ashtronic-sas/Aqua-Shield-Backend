...
from app.services.employee_place import create_employee_place, get_employee_place_all, get_employee_place_all_by_id_db, delete_employee_place_db
...
@router.get("/{employee_id}", response_model=list[EmployeePlaceResponse], dependencies=[Depends(verify_token)])
def get_employee_place__by_id(employee_id: int, db: Session = Depends(get_db)):
    return get_employee_place_all_by_id_db(db, employee_id)
